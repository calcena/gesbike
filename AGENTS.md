...
### 10.1 Restaurar BD (desde un `.zip`, volúmenes `.7z.001` o partes `.gz.001` dejados en el servidor)
El usuario deja un `.zip` (o un 7z partido en volúmenes `.7z.001`, `.7z.002`, ...) en el servidor, y al pulsar el botón el programa **extrae** la base de datos en la misma ubicación y **borra el archivo (y sus partes)** tras hacerlo. El formato 7z en volúmenes permite subir el backup por partes pequeñas a hostings con límite de subida bajo (el 7z además comprime mejor que el zip, p. ej. ~7 MB frente a ~10,7 MB).

**IMPORTANTE (hostings sin binario 7z)**: PHP no puede descomprimir LZMA2 nativamente y no existe librería PHP pura de 7z. Si el servidor no tiene `7z`/`7za`/`7zr` (p7zip), el endpoint responde error pidiendo el uso de **partes `.gz.001/.002/...`**: el gzip es un flujo continuo (concatenar las partes reconstruye el `.gz`) y se descomprime con las funciones `gz*` de PHP (zlib, siempre disponible), en streaming (1 MB por lectura, sin cargar todo en memoria). Creación local (Windows): 7-Zip → "Añadir al archivo" → formato **gzip** sobre `app.db` → `app.db.gz`; partirlo con PowerShell (`$in=[IO.File]::OpenRead('app.db.gz'); $b=New-Object byte[] (2MB); $i=1; while(($n=$in.Read($b,0,$b.Length))-gt 0){$o=[IO.File]::Create(('app.db.gz.{0:D3}'-f $i)); $o.Write($b,0,$n); $o.Close(); $i++}; $in.Close()`). Cada parte debe quedar por debajo del límite de subida del hosting. También se acepta `.bz2.NNN` si `bzopen` existe (mejor compresión), y `.gz`/`.bz2` sueltos (sin partes).
...
- **Comportamiento**:
  1. Busca `*.zip`, juegos de partes `*.7z.NNN` y `*.gz.NNN` (y `.bz2.NNN` si `bzopen` existe) en `database/` (la misma ubicación que la BD actual) y, si no hay, en `database/backups/`. **Se elige el candidato MÁS RECIENTE por fecha de modificación** (no el alfabético: evita que un zip legacy como `app.db.zip` tracked en git prevalezca sobre el que acaba de dejar el usuario).
  2. **Zip**: lo abre con `ZipArchive`. **7z**: ensambla las partes en orden numérico (`001 < 002 < ... < 010 < 011`) en un `.7z` único, verifica integridad con `7z t` (detecta partes incompletas/corruptas), y extrae con el binario `7z`/`7za`/`7zr` (p7zip) a un directorio temporal. Si no hay binario 7z en el servidor, devuelve error pidiendo el uso de partes `.gz`. **Gzip/Bzip2**: ensambla las partes en un `.gz`/`.bz2` único y descomprime en streaming con `gzopen`/`gzread` (o `bzopen`/`bzread`) directamente a `database/{nombreDb}` (el nombre debe ser `app.db.gz`, `gesbike.db.gz` o `comandos_voz.db.gz`); tolera también el `.gz`/`.bz2` suelto sin partes.
  3. En todos los casos extrae **solo** `app.db`, `gesbike.db`, `comandos_voz.db` (en `database/`) y las entradas `hist/**` (en la raíz del proyecto, preservando subcarpetas; en 7z se tolera un prefijo de carpeta tipo `backup/`). No extrae otros ficheros, por seguridad.
  4. **Unlink previo del destino** antes de sobrescribir: `ZipArchive::extractTo`/`copy`/`fopen(w)` pueden fallar con EPERM/EACCES al sobrescribir ficheros existentes creados por otro usuario (p. ej. permisos `644` owner dccsoft y el proceso web es `www-data`).
  5. Tras extraer borra del servidor el `.zip` (o las partes `.7z.*`/`.gz.*` y el archivo ensamblado) y el directorio temporal, y normaliza permisos `0664` en lo extraído (el grupo `www-data` debe poder escribir el histórico).
  6. Devuelve JSON `{ success, message, archivos }` (Siempre JSON puro: descarta buffers de salida al inicio, fuerza `display_errors=0` y limpia el buffer antes de emitir la respuesta, para hostings que escupen warnings antes del JSON). Si no hay candidato o no contiene `app.db`, devuelve error. **Limpieza robusta**: el borrado de partes/origen usa `borrar_si_puede()` (unlink → chmod 0644 + unlink → truncar a 0 bytes); los archivos que no se puedan eliminar (permisos del FTP) se listan en el JSON (`restantes`) para borrarlos manualmente.
...
### 10.3 Backup automático (`helpers/backup.php`)
`crearBackup()` (en `services/helpers/helper.js`, llamada con `await` tras guardar/borrar en compras, recambios, mantenimientos y rutas) hace `fetch(POST)` a `helpers/backup.php`. **El endpoint NO hace el backup de forma síncrona**: lanza un proceso PHP CLI separado (`php helpers/backup.php` con `exec`/`popen`/`proc_open` vía `lanzar_backup_segundo_plano()`, binario resuelto con `php_cli_binary()`) y responde al instante `{ success, message: 'Backup iniciado en segundo plano' }`. Esto evita que el backup (~8 s en una BD de 17 MB) bloquee al servidor y congele la paginación u otras peticiones. Si el hosting no permite ejecutar procesos, hace fallback al backup síncrono histórico.
- **Script dual CLI/web** (patrón de `partir_backup.php`): el guard `if (php_sapi_name() === 'cli' && realpath($argv[0] ?? '') === __FILE__)` ejecuta `realizarBackupSQLite()` y registra el resultado en `database/backups/backup_cli.log` (fallback a `sys_get_temp_dir()`), sin tocar la BD vía web.
- **IMPORTANTE (guardado de rutas)**: en `guardarRutaManual` (`services/rutas/ruta.js`), tras guardar se ejecuta `await getRutasByVehiculo(); await getKmsDetail(); crearBackup();` — el backup va **sin `await`** y después de refrescar la UI, de modo que `kms_realizados` y la paginación respondan al momento (antes, `await crearBackup()` encolaba ~8 s todas las peticiones en servidores de un solo hilo, dejando la paginación "congelada" tras editar una ruta en la última página).
